/* This file is concerned with the user interface of the application.
 *
 * The functions in this file are responsible for hiding and
 * displaying UI elements based on the user's actions.
 * They also take the user's chosen input and settings, and feed them
 * into the back-end functions to perform image transformations.
 */
...
var	rect = {},
	...
	var orig = document.querySelector('#Original').getContext('2d');
...
$(function() {
	...
	function handleFiles(event) {
		...
		reader.onload = function(event) {
			...
			//Check if the file is a dicom file and handle it differently than regular images
			var extension = files[0].name.split('.').pop().toLowerCase();
			if(extension === 'dcm') {
				convertMonochrome2Dicom(event);
				anImage.src = orig.canvas.toDataURL("image/png");
			} else {
				anImage.src = event.target.result;
			}

				/* Setup the applicationa after loading the image.
				 */
				$(anImage).on('load', function () {
					...
					result.canvas.width = orig.canvas.width = imgWidth;
					result.canvas.height = orig.canvas.height = imgHeight;

					//Set the original image to the uploaded image
					orig.drawImage(anImage, 0, 0);
